...
<div id="pagination">
	<ul class="pagination justify-content-center">
		<c:if test="${pageMaker.prev }">
			<li><a href="/place/list?page=${pageMaker.startPage -1 }">이전</a></li>
		</c:if>
		<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }"
			var="idx">
			<li class="page-item"><c:choose>
					<c:when test="${pageMaker.criteria.page == idx}">
						<a class="pagination selected" href="/place/list?page=${idx }">${idx }</a>
					</c:when>
					<c:otherwise>
						<a class="pagination" href="/place/list?page=${idx }">${idx }</a>
					</c:otherwise>
				</c:choose></li>
		</c:forEach>
		<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
			<li><a href="/place/list?page=${pageMaker.endPage + 1 }">다음</a></li>
		</c:if>
	</ul>
</div>
</div>
